...
export const HighImpactHero: React.FC<Page['hero']> = ({
  ...
  const imageStyles = {
    none: '',
    opacity: 'opacity-[.8]',
    blur: 'blur opacity-75',
  }

  const imageStyleClass = imageStyles[imageStyle] || imageStyles['none']
    ...
    <div className={`${classes.hero} flex justify-center items-center cs_parallax`}>
      <div className={classes.background}>
        {media && typeof media === 'object' && (
          <Media
            resource={media}
            className={`${classes.backgroundImage} ${imageStyleClass}`}
            priority
          />
        )}
      </div>
      <div className="container">
        <div>
          <div className="flex flex-row items-center !text-secondary text-lg leading-loose mb-0">
            <svg
              width="120"
              height="12"
              viewBox="0 0 134 12"
          ...
          <h1 className={`${classes.title} animate-fade-up animate-once animate-ease-in`}>
            {title}
          </h1>
